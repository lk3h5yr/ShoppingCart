...
public class CartInfoServiceImpl implements ICartInfoService {
	...
	public List<GoodsInfo> queryAllGoodsInfo() {
		...
		try {
			...
			throw new CartsProcessException("外部API系統錯誤! URL: " + allGoodsesUrl, ExceptionStatus.INPUT_PARAMETER_ERROR, HttpStatus.BAD_REQUEST.value());
		}
	...
	public Map getGoodsBygoodsId(String goodsId) {
		...
		try {
			...
			logger.error(e.getMessage());
			throw new CartsProcessException("外部API系統錯誤! URL: " + goodsByGoodsIdUrl, ExceptionStatus.INPUT_PARAMETER_ERROR, HttpStatus.BAD_REQUEST.value());
		}
